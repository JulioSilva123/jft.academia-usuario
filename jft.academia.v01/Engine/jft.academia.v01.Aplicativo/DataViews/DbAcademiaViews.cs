using jft.academia.v01.Aplicativo.DataViews.RepositorysViews;
using System;
using System.Collections.Generic;
using System.Text;

namespace jft.academia.v01.Aplicativo.DataViews
{
    public class DbAcademiaViews
    {

        public DbAcademiaViews() { }



        //public AtividadesRepositoryView Atividades { get; set; }

        //private Data.Repositorys.GruposReporitory _Grupos;
        //public Data.Repositorys.GruposReporitory Grupos
        //{
        //    get
        //    {
        //        if (_Grupos is null)
        //        {
        //            _Grupos = new Data.Repositorys.GruposReporitory();
        //        }

        //        return _Grupos;

        //    }
        //}


        private AtividadesRepositoryView _Atividades;
        public AtividadesRepositoryView Atividades
        {
            get
            {
                if (_Atividades is null)
                {
                    _Atividades = new AtividadesRepositoryView();
                }

                return _Atividades;

            }
        }


        private Atividades_GruposRepositoryView _Atividades_Grupos;
        public Atividades_GruposRepositoryView Atividades_Grupos
        {
            get
            {
                if (_Atividades_Grupos is null)
                {
                    _Atividades_Grupos = new Atividades_GruposRepositoryView();
                }

                return _Atividades_Grupos;

            }
        }



    }
}
